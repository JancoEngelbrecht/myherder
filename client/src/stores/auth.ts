...
import db, { initDb, closeDb } from '../db/indexedDB'
import {
  initialSync,
  isOfflineError,
  pushChanges,
...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function setSession(data: any) {
    ...
    const featureFlagsStore = useFeatureFlagsStore()
    featureFlagsStore.fetchFlags().catch(() => {})
    const { useSpeciesStore } = await import('./species')
    const speciesStore = useSpeciesStore()
    speciesStore.fetchAll().catch(() => {})
    initialSync().catch(() => {})
    // Eagerly fetch animals after login
    const { useAnimalsStore } = await import('./animals')
  ...
  async function login(username: string, password: string, farmCode?: string) {
    try {
  ...
  async function loginPin(username: string, pin: string, farmCode: string) {
    try {
   ...
   */
  async function switchFarm(farmId: string, { force = false } = {}) {
    // Flush pending sync queue before switching farms
    const allPending = await getPending()
    const recoverable = allPending.filter((e: any) => e.attempts < 5)
    if (recoverable.length > 0) {
      ...
      const remaining = (await getPending()).filter((e: any) => e.attempts < 5).length
      if (remaining > 0 && !force) {
        const err: any = new Error('PENDING_SYNC')
  ...
  async function enterFarm(farmId: string, { force = false } = {}) {
