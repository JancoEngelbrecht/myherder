 ...
 *
 * @param {object} cow - cow/animal record with sex, dob, life_phase_override
 * @param {object} [breedType] - breed type with calf_max_months, heifer_min_months, young_bull_min_months
 * @param {object} [speciesLifePhases] - species.config.life_phases { female: [...], male: [...] }
 */
export function computeLifePhase(cow, breedType = null, speciesLifePhases = null) {
  ...
  if (speciesLifePhases) {
    const phases = cow.sex === 'male' ? speciesLifePhases.male : speciesLifePhases.female
    if (!phases?.length) return cow.sex === 'male' ? 'bull' : 'cow'
  ...
  if (ageMonths === null) return cow.sex === 'male' ? 'bull' : 'cow'
