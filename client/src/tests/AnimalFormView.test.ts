...
function createWrapper(opts = {}) {
  api.get.mockImplementation((url) => {
    ...
    if (typeof url === 'string' && url.startsWith('/animals/')) {
  ...
  return mount(AnimalFormView, { global: { stubs }, ...opts })
}
...
describe('AnimalFormView', () => {
  it('renders create form with empty fields when no route id', async () => {
    ...
    expect(wrapper.find('.animal-form').exists()).toBe(true)
    const tagInput = wrapper.find('input[type="text"]')
    expect(tagInput.element.value).toBe('')
  })

  it('renders edit form pre-filled with animal data when route id present', async () => {
    mockRouteParams = { id: 'animal-1' }
    mockRoutePath = '/animals/animal-1/edit'
  ...
  it('renders sire and dam AnimalSearchDropdowns', async () => {
    const wrapper = createWrapper()
    await flushPromises()
  ...
  it('submit calls animalsStore.create for new animal', async () => {
    api.post.mockResolvedValue({ data: { id: 'new-animal-1', tag_number: 'TAG-NEW' } })
  ...
  it('cancel button navigates to animal list for new animal', async () => {
    const wrapper = createWrapper()
    await flushPromises()
    ...
    expect(mockPush).toHaveBeenCalledWith('/animals')
  })
})

// ── Offspring mode tests ──────────────────────────────────────────────────────

describe('AnimalFormView — offspring mode', () => {
