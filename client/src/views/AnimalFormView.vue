<template>
  <div class="page">
    <AppHeader
      :title="pageTitle"
      :show-back="true"
      :back-to="isEdit ? `/animals/${route.params.id}` : '/animals'"
    />
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    try {
      const animal = await animalsStore.fetchOne(route.params.id)
      Object.assign(form, {
        tag_number: animal.tag_number || '',
        name: animal.name || '',
        breed_type_id: animal.breed_type_id || null,
        dob: animal.dob ? animal.dob.substring(0, 10) : '',
        sex: animal.sex || 'female',
        status: animal.status || 'active',
        notes: animal.notes || '',
        sire_id: animal.sire_id || null,
        dam_id: animal.dam_id || null,
        is_external: animal.is_external || false,
        purpose: animal.purpose || null,
        life_phase_override: animal.life_phase_override || null,
      })
    } catch {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await animalsStore.update(route.params.id, payload)
      router.replace(`/animals/${route.params.id}`)
    } else {
      const animal = await animalsStore.create(payload)
      ...
      if (isOffspringMode.value) {
        if (offspringAction.value === 'next' && !isLastOffspring.value) {
          ...
          form.notes = ''
          form.life_phase_override = null
          await router.replace({ path: '/animals/new', query: q })
        } else {
          // Save & Done — navigate to dam detail
          const damId = route.query.dam_id
          router.replace(damId ? `/animals/${damId}` : '/animals')
        }
      } else {
...
function handleSkipOffspring() {
  const damId = route.query.dam_id
  router.replace(damId ? `/animals/${damId}` : '/breed')
}
