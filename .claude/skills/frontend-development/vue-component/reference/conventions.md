...
### Markdown Files and Vue Template Compilation

When using `unplugin-vue-markdown` with `@vitejs/plugin-vue` (configured with `include: [/\.vue$/, /\.md$/]`), `.md` files are treated as Vue SFCs by the template compiler.

- **Problem**: HTML elements like `<details>` and `<summary>` in `.md` files cause "Element is missing end tag" build errors because the Vue compiler cannot parse them as valid Vue template elements
- **Fix**: Replace `<details>`/`<summary>` blocks with plain markdown headings (`###`)
- **Rule**: Avoid raw HTML block-level elements in any `.md` file that is processed by the Vue markdown plugin. Stick to standard markdown syntax
- **Discovered in**: PR #2716 (feat/new-docs-drawer)
