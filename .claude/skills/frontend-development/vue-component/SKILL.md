...
| **2. Create PLAN.md** | Document implementation plan with steps, file changes, decisions. **MUST include a Test Strategy section** — apply the Test Decision Framework upfront and include test phases (unit tests for composables/helpers, E2E tests if criteria are met) in the plan's implementation phases. | `PLAN.md` in feature directory |
