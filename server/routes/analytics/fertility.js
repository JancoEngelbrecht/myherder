...
router.get('/breeding-overview', async (req, res, next) => {
  try {
    ...
    const pregnantIds = new Set(
      activeFemales.filter((c) => c.status === 'pregnant').map((c) => c.id)
    )

    // Dry = status dry
    const dryIds = new Set(activeFemales.filter((c) => c.status === 'dry').map((c) => c.id))
    ...
    for (const cow of activeFemales) {
      if (
        ...
        !dryIds.has(cow.id) &&
        !bredAwaitingIds.has(cow.id) &&
        !cowsWithEventsSet.has(cow.id)
      ) {
        heiferNotBredIds.add(cow.id)
      }
    }

    const not_pregnant_count = activeFemales.filter(
      (c) =>
        !pregnantIds.has(c.id) &&
        !dryIds.has(c.id) &&
        !bredAwaitingIds.has(c.id) &&
        !heiferNotBredIds.has(c.id)
    ).length
    ...
    const pregnancy_rate =
      eligibleFemales > 0 ? round2((pregPositiveCount / eligibleFemales) * 100) : null
    ...
    const avg_services_per_conception =
      conceptionCount > 0 ? round2(totalServices / conceptionCount) : null
...
router.get('/conception-rate', async (req, res, next) => {
  try {
    ...
    const by_month = Object.entries(monthlyData)
      ...
      .map(([month, data]) => ({
        month,
        rate: data.total > 0 ? Math.round((data.conceptions / data.total) * 1000) / 10 : 0,
        total: data.total,
...
router.get('/calving-interval', async (req, res, next) => {
  try {
    ...
    const avg_calving_interval_days =
      intervals.length > 0
