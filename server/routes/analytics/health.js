...
router.get('/mastitis-rate', async (req, res, next) => {
  try {
    ...
    const months = rows.map((r) => {
      ...
      const rate = herd_size > 0 ? round2((cases / herd_size) * 100) : 0
      return { month: r.month, rate, cases, herd_size }
    })

    const avg_rate =
      months.length > 0 ? round2(months.reduce((sum, m) => sum + m.rate, 0) / months.length) : 0
  ...
  try {
    ...
    const rows = await db('health_issues')
      ...
      .select(
        ...
        db.raw(
          "SUM(CASE WHEN status = 'resolved' AND resolved_at IS NOT NULL THEN 1 ELSE 0 END) as resolved"
        )
      )
      .groupByRaw(monthExpr('observed_at'))
      .orderBy('month')
