...
exports.seed = async function (knex) {
  ...
  function seededJitter(base, pct) {
  ...
  function seasonalMilkFactor(month) {
    ...
    const factors = [0, 1.1, 1.12, 1.05, 0.95, 0.85, 0.78, 0.75, 0.8, 0.9, 0.98, 1.05, 1.08]
  ...
  function breedBaseLitres(breed) {
    switch (breed) {
      case 'Holstein':
        return rand(12, 16) // high producers
      case 'Jersey':
        return rand(8, 12) // less volume, richer milk
      case 'Ayrshire':
        return rand(10, 13)
      case 'Nguni':
        return rand(4, 7) // dual-purpose, lower yield
      case 'Brahman':
        return rand(3, 5) // beef breed, minimal milking
      default:
        return rand(8, 12)
    }
  }
  ...
  while (cursor <= today) {
    ...
    for (const cow of milkingCows) {
      ...
      const morningLitres =
        Math.round(seededJitter(baseLitres * 0.55 * seasonFactor, 0.15) * 100) / 100
      const afternoonLitres =
        Math.round(seededJitter(baseLitres * 0.45 * seasonFactor, 0.15) * 100) / 100
  ...
  const treatmentNotes = {
    ...
    respiratory: [
      ...
      'Isolated from herd',
      'Temperature 40.2°C',
      'Mild pneumonia suspected',
    ],
    digestive: [
      'Off feed for 2 days',
      'Rumen sounds reduced',
      'Possible grain overload',
      'Appetite returning',
    ],
    fever: [
      'Temperature 40.5°C',
    ...
    eye: [
      'Pinkeye, left eye cloudy',
      'Tearing and swelling',
      'Fly irritation likely cause',
      'Eye patch applied',
    ],
    bad_milk: [
  ...
  const existingEvents = await knex('breeding_events').select('cow_id', 'event_type', 'event_date')
  ...
  for (const e of existingEvents) {
    ...
    if (e.event_type === 'calving') {
      const evtDateStr =
        e.event_date instanceof Date ? e.event_date.toISOString() : String(e.event_date)
  ...
  for (const [cowId, dates] of Object.entries(calving001)) {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      ...
      const hasExistingAI = existingEvents.some(
        (e) =>
          e.cow_id === cowId &&
          (e.event_type === 'ai_insemination' || e.event_type === 'bull_service') &&
          e.event_date > sorted[i] &&
          e.event_date < sorted[i + 1]
      )
