...
function drawCow(pixels, canvasW, canvasH, cx, cy, size, fr, fg, fb) {
  ...
  const legOffsets = [-Math.round(30 * s), -Math.round(10 * s), Math.round(10 * s), Math.round(28 * s)];
  ...
  for (let dy = -snoutH; dy <= snoutH; dy++) {
    ...
    fillRect(pixels, canvasW, canvasH, snoutX - hw, snoutY + dy, hw * 2, 1,
      Math.min(255, fr + 40), Math.min(255, fg + 40), Math.min(255, fb + 40));
  }

  // Ear
  fillCircle(pixels, canvasW, canvasH,
    hcx + Math.round(8 * s), hcy - Math.round(15 * s),
    Math.round(7 * s), fr, fg, fb);

  // Horns (two small arcs)
  fillCircle(pixels, canvasW, canvasH,
    hcx - Math.round(4 * s), hcy - Math.round(16 * s),
    Math.round(5 * s), fr, fg, fb);
  ...
  for (let dy = 0; dy <= Math.round(12 * s); dy++) {
    ...
    fillRect(pixels, canvasW, canvasH, cx - Math.round(10 * s) - hw / 2, udderY + dy, hw, 1,
      Math.min(255, fr + 30), Math.min(255, fg + 20), Math.min(255, fb + 20));
  }
...
function generateScreenshot(w, h, form) {
  ...
  if (form === 'wide') {
    ...
    drawText(pixels, w, h, title,
      32, Math.round((64 - 7 * titleScale) / 2),
      titleScale, ...WHT);

    // Tagline under nav bar
    const tagScale = 2;
    const tag = 'Dairy Farm Management System';
    drawText(pixels, w, h, tag,
      32, 90, tagScale, ...TXT);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      drawText(pixels, w, h, cardLabels[i],
    ...
    drawCow(pixels, w, h,
      Math.round(w * 0.85), Math.round(h * 0.60),
    ...
    // Cow illustration
    const cowY = 270;
    drawCow(pixels, w, h, Math.round(w / 2), cowY,
      Math.round(w * 0.55), ...GRN);

    // Tagline
    const tagScale = 2;
    const line1 = 'Dairy Farm';
    const line2 = 'Management System';
    drawText(pixels, w, h, line1,
      Math.round((w - textWidth(line1, tagScale)) / 2),
      cowY + Math.round(w * 0.55 * 0.55) + 20, tagScale, ...TXT);
    drawText(pixels, w, h, line2,
      Math.round((w - textWidth(line2, tagScale)) / 2),
      cowY + Math.round(w * 0.55 * 0.55) + 40, tagScale, ...TXT);
